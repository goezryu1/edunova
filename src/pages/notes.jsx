...
import { Plus, Trash2, Sparkles, RefreshCw, Search, ArrowLeft } from 'lucide-react'
...
export default function Notes() {
  ...
  // Track viewport so we can switch to a stacked, single-pane layout on mobile
  ...
  // On mobile, decide whether we're showing the LIST or a DETAIL/FORM pane
  const showingDetail = isMobile && (showForm || selected)
    ...
    <div style={{
      ...
      {showingDetail || !isMobile ? (
        <div style={{ flex:1, borderRadius:18, background:'rgba(255,255,255,0.028)', border:'1px solid rgba(255,255,255,0.07)', backdropFilter:'blur(24px)', overflow:'hidden', display:'flex', flexDirection:'column' }}>
          {/* Back button — mobile only, shown whenever we're on the detail/form pane */}
          {isMobile && (showForm || selected) && (
            <button
              onClick={() => { setShowForm(false); setSelected(null) }}
              style={{ display:'flex', alignItems:'center', gap:6, padding:'12px 16px', fontSize:12.5, color:'var(--text-2)', borderBottom:'1px solid rgba(255,255,255,0.06)' }}
            >
          ...
          {showForm ? (
            ...
            <div style={{ padding: isMobile ? '18px 16px' : '24px', overflowY:'auto', flex:1 }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14 }}>
                <div>
                  <h2 style={{ fontFamily:'var(--ff-display)', fontSize: isMobile ? 17 : 20, fontWeight:800, marginBottom:6 }}>{selected.title}</h2>
                ...
                <button onClick={()=>remove(selected.id)} style={{ display:'flex', alignItems:'center', gap:5, fontSize:12, color:'#ff4d6d', transition:'opacity 0.15s', flexShrink:0 }}><Trash2 size={13} /> Delete</button>
              </div>
