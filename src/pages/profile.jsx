      ...
      <div className="glass-card" style={{ borderRadius: '0 0 16px 16px', padding: '0 24px 24px', borderTop: 'none' }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 16, marginBottom: 20, paddingTop: 0, transform: 'translateY(-32px)', marginBottom: -8 }}>
          ...
          <div style={{ flex: 1, paddingBottom: 4 }}>
            <div style={{ fontFamily: 'var(--font-display)', fontSize: 20, fontWeight: 800, letterSpacing: '-0.5px' }}>{user.name}</div>
        ...
        {editing ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 20, paddingTop: 16 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <div>
                <label style={{ fontSize: 10, color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', display: 'block', marginBottom: 5, textTransform: 'uppercase', letterSpacing: 0.5 }}>Display Name</label>
                <input className="input-field" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} />
              </div>
              <div>
                <label style={{ fontSize: 10, color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', display: 'block', marginBottom: 5, textTransform: 'uppercase', letterSpacing: 0.5 }}>Bio</label>
                <input className="input-field" value={form.bio} onChange={e => setForm(f => ({ ...f, bio: e.target.value }))} />
              </div>
            </div>

            <div>
              <label style={{ fontSize: 10, color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', display: 'block', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 0.5 }}>Avatar</label>
            ...
            <div>
              <label style={{ fontSize: 10, color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', display: 'block', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 0.5 }}>Accent Color</label>
          ...
          <div style={{ paddingTop: 16 }}>
            {user.bio && <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 10 }}>{user.bio}</p>}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 20 }}>
          {[
            ...
            { icon: <Flame size={16} />, val: `${user.streak}d`, label: 'Streak', color: 'var(--neon-amber)' },
            { icon: <Trophy size={16} />, val: `${earned.length}`, label: 'Achievements', color: 'var(--neon-amber)' },
            { icon: <BookOpen size={16} />, val: gpa, label: 'GPA', color: 'var(--neon-green)' },
          ].map((s, i) => (
            <div key={i} style={{ textAlign: 'center', padding: '14px 8px', borderRadius: 12, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)' }}>
              ...
              <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 20, color: s.color, letterSpacing: '-1px' }}>{s.val}</div>
              <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 2, fontFamily: 'var(--font-mono)' }}>{s.label}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ padding: '14px 16px', borderRadius: 12, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            <span style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 13 }}>Level {user.level}</span>
            <span style={{ fontSize: 11, fontFamily: 'var(--font-mono)', color: 'var(--text-muted)' }}>{user.xp} / {user.xpNext} XP</span>
          </div>
        ...
        {earned.length > 0 && (
          <>
            <div className="divider" />
            <div style={{ fontSize: 13, fontWeight: 600, fontFamily: 'var(--font-display)', marginBottom: 10 }}>Badges</div>
